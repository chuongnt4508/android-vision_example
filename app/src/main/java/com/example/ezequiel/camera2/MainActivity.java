...
import com.example.ezequiel.camera2.others.MyFaceDetector;
...
public class MainActivity extends AppCompatActivity {
                ...
                public void onClick(View v) {
                    ...
                    if (isRecordingVideo) {
                        ...
                        if (useCamera2) {
                            ...
                            if (mCameraSource != null)
                                mCameraSource.recordVideo(cameraSourceVideoStartCallback, cameraSourceVideoStopCallback, cameraSourceVideoErrorCallback);
                        }
                    }
                }
            });
    ...
    private void createCameraSourceFront() {
        ...
        myDetector = new MyFaceDetector(previewFaceDetector);
        if (myDetector.isOperational()) {
            GraphicFaceTracker tracker = new GraphicFaceTracker(mGraphicOverlay);
            LargestFaceFocusingProcessor largest = new LargestFaceFocusingProcessor.Builder(myDetector, tracker).build();
            myDetector.setProcessor(largest);
        } else {
            Toast.makeText(context, "FACE DETECTION NOT AVAILABLE", Toast.LENGTH_SHORT).show();
        }

        if (useCamera2) {
            mCamera2Source = new Camera2Source.Builder(context, myDetector)
    ...
    private void xxx() {
        try {
            ...
            String path = Environment.getExternalStorageDirectory() + "/" + Environment.DIRECTORY_MOVIES + "/" + "20200102_140214.mp4";
...
//                Bitmap bm = AndroidUtil.toBitmap(picture);
...
//                });
                ...
                Frame f = new Frame.Builder().setBitmap(rotatedBitmap).build();
                SparseArray<Face> faces = detector.detect(f);
                for (int i = 0; i < faces.size(); i++) {
                    Face face = faces.valueAt(i);
                    final float y = face.getEulerY();
                    final float z = face.getEulerZ();
...
//                    }
    ...
    private void savePicture() throws Exception {
        ...
        try (FileOutputStream fos = new FileOutputStream(file)) {
            boolean compressResult = myDetector.getFrameBitmap().compress(Bitmap.CompressFormat.PNG, 100, fos);
            if (compressResult) {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == REQUEST_CAMERA_PERMISSION) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Log.d("DEBUG::::::::::", "CAMERA PERMISSION REQUIRED");
                finish();
            }
        }
        if (requestCode == REQUEST_STORAGE_PERMISSION) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Log.d("DEBUG::::::::::", "STORAGE PERMISSION REQUIRED");
            }
        }
    }
